                    ...
                    >
                      {imagePreviews.length > 0 ? (
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', width: '100%', padding: '24px 0 12px' }} onClick={e => e.stopPropagation()}>
                          <div style={{ position: 'relative', height: 150, width: (Math.min(imagePreviews.length, 5) - 1) * 50 + 95 }}>
                            {imagePreviews.map((src, i) => {
                              ...
                              return (
                                <div key={i} style={{ position: 'absolute', left: i * 50, bottom: 0, zIndex: i, transform: `rotate(${rot}deg) translateY(${ty}px)`, transformOrigin: 'bottom center', transition: 'transform 0.2s' }}
                                ...
                                >
                                  <img src={src} alt={`Preview ${i + 1}`} style={{ width: 95, height: 118, objectFit: 'cover', borderRadius: 12, border: '2.5px solid #fff', boxShadow: '0 8px 22px rgba(0,0,0,0.18)', display: 'block' }} />
                                  <button
                                    type="button"
